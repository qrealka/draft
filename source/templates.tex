...
in a \grammarterm{qualified-id} formed by a \grammarterm{class-head-name} (Clause~\ref{class}) or
\grammarterm{enum-head-name}~(\ref{dcl.enum}),
...
\grammarterm{class-or-decltype} (Clause~\ref{class.derived}),
...
a member of an unknown specialization~(\ref{temp.dep.type}),
...
then \tcode{AA} is the pack expansion \tcode{PP...}~(\ref{temp.variadic});
...
default arguments and \grammarterm{noexcept-specifier}{s} of function
templates and default arguments and \grammarterm{noexcept-specifier}{s} of
member functions of class templates are considered definitions;
each default argument or \grammarterm{noexcept-specifier} is a separate
...
\item In a \grammarterm{using-declaration}~(\ref{namespace.udecl});
...
\item
an \grammarterm{identifier} associated by name lookup with a decomposition
declaration~(\ref{dcl.decomp}) whose \grammarterm{brace-or-equal-initializer}
is type-dependent,
