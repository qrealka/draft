...
namespace std {
  ...
  template <class charT, class traits>
    @\textit{T13}@ quoted(basic_string_view<charT, traits> s,
    @\itcorr@           charT delim = charT(’"’), charT escape = charT(’\\’));

  template <class charT, class traits, class Allocator>
    @\textit{T14}@ quoted(basic_string<charT, traits, Allocator>& s,
    @\itcorr@           charT delim = charT('"'), charT escape = charT('\\'));
}
\end{codeblock}
...
are only be provided on systems where \tcode{filesystem::path::value_type}~(\ref{class.path}) is not \tcode{char}.
\begin{note}
...
namespace std {
  ...
  public:
    ...
    basic_filebuf<charT, traits>* open(const filesystem::path::value_type* s,
                                       ios_base::openmode mode);  // wide systems only; see \ref{fstream.syn}
    basic_filebuf<charT, traits>* open(const string& s,
                                       ios_base::openmode mode);
    basic_filebuf<charT, traits>* open(const filesystem::path& s,
                                       ios_base::openmode mode);
...
\begin{note}
The generic pathname grammar~(\ref{path.generic}) defines \grammarterm{directory-separator} as one or more slashes and \grammarterm{preferred-separator}{s}.
\end{note}
...
namespace std::filesystem {
  ...
  public:
    ...
    // \ref{enum.path.format}, enumeration \tcode{format}
    enum format;
...
\nontermdef{root-name}\br
    \textnormal{An operating system dependent name that identifies the starting location for pathname resolution.
...
\pnum
Except in \grammarterm{root-name},
...
\pnum
The filename \grammarterm{dot}~(\ref{fs.def.filename}) is treated as a reference to the current directory.
The filename \grammarterm{dot-dot}~(\ref{fs.def.filename}) is treated as a reference to the parent directory.
...
\pnum
If root-name is otherwise ambiguous,
...
using either a pathname in the generic format~(\ref{path.generic})
or a pathname in the native format~(\ref{fs.def.native}).
Such an argument is taken to be in the generic format if and only if
it matches the generic format but is not acceptable to the operating system
as a native path.
...
\pnum
\begin{note}
A path stores a native format pathname~(\ref{path.native.obs})
...
Implementations are strongly encouraged not to modify member function arguments
...
has the original value of \tcode{source}~(\ref{path.fmt.cvt})
...
\begin{itemdescr}
\pnum
\effects Let \tcode{s} be the effective range of \tcode{source}~(\ref{path.req})
or the range \range{first}{last}, with the encoding converted if required~(\ref{path.cvt}).
Finds the detected-format of \tcode{s}~(\ref{path.fmt.cvt})
...
\pnum
\effects
Let \tcode{s} be the effective range of \tcode{source}~(\ref{path.req})
or the range \range{first}{last}, with the encoding converted if required~(\ref{path.cvt}).
Finds the detected-format of \tcode{s}~(\ref{path.fmt.cvt})
...
\item If \tcode{p.has_root_directory()},
  ...
  Otherwise, if \tcode{has_filename() || (!has_root_directory() \&\& is_absolute())},
...
\tcode{path("//host")/"foo"} and \tcode{path("//host/")/"foo"}
both equal \tcode{"//host/foo"}.
...
\effects
  \begin{itemize}
\item Any existing \tcode{extension()(}\ref{path.decompose}\tcode{)} is removed from the
...
\pnum
\effects
If \tcode{root_name()!=base.root_name() || is_absolute()!=base.is_absolute() || (!has_root_directory() \&\& base.has_root_directory())},
returns \tcode{path()}.
...
\item let \tcode{n} be the number of filename elements in \range{b}{base.end()}
...
\rSec3[enum.path.format]{Enum \tcode{path::format}}
...
is a bitmask type~(\ref{bitmask.types}) that specifies bitmask constants used to
...
%% There are too many tables in the preceding text, so we should flush them
...
class \tcode{directory_iterator}~(\ref{class.directory_iterator})
...
\begin{itemdescr}
\pnum
\effects Stores the current value for any cached attribute values of the file \tcode{p} resolves to.
If an error occurs, an error is reported~(\ref{fs.err.report})
and the value of any cached attributes is unspecified.
...
\pnum
\begin{note}
Implementations of \tcode{directory_iterator}~(\ref{class.directory_iterator})
...
path and any cached attribute values~(\ref{class.directory_entry})
for each file in a directory or in an \impldef{type of a directory-like file} directory-like file type.
...
shall store the cached attribute values, if any, described in \ref{class.directory_entry}
...
For exposition class \tcode{directory_iterator} is shown in \ref{class.directory_entry}
...
as \tcode{p} according to the operating system~(\ref{fs.conform.os}).
...
such as hard disks, consider \tcode{canonical}~(\ref{fs.op.canonical}).
\end{note}
...
file type, then the effects are \impldef{effect of \tcode{filesystem::copy}}.
...
\begin{itemdescr}
\pnum
  \requires One and only one of the \tcode{perm_options} constants
...
\pnum
  ...
  \tcode{perm_options} \tcode{opts} with a value of \tcode{perm_options::replace}.
...
\item Otherwise, if the attributes indicate an
